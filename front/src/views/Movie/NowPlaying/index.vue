					...
					<li class="uls licontent" v-for="item in arr1" :key="item.id" v-lazy="item">
						...
						<div class="liright">
							...
							<router-link to="/movie/detail">
								...
								<p @click="goDetail(item.id,item.name)">{{item.pd}}</p>
...
export default {
	...
	methods: {
		...
		getData() {
			var that = this;
			axios.get('/static/movie_list.json', { baseURL: 'http://localhost:8080' }, { params: { pageNum: that.page, pageSize: that.pageSize } })　　　　.then(function(data) {
				if (data.data.success) {　　　　　　　　 //这一步是判断你当前请求的这一页数据是不是最后一页，如果是最后一页就不能请求了（这个根据后端给的接口判断，只要能判断出就行了，如果是最后一页给that.noDate=true）
