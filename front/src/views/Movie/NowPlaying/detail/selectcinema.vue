...
export default {
	...
	if(!this.$store.state.isLogin) {
		...
		this.indexsarr.forEach((item)=> {
			orderSet.push([item.p, item.l]);
		})
	

		let order = {
			shopName: this.ObjResult.cinema[0].nm,
			orderType: 'Film',
			price:   this.ObjResult.price,
			orderInfo: [
				{
					orderPicurl: this.ObjResult.movie[0].jpgUrl.replace('w.h', '128.180'),
					orderName:  this.ObjResult.movie[0].nm,
					orderCount: this.indexsarr.length,
					currentPrice:  35,
					orderSet: orderSet,
				}
			]
			...
			console.log(order);
		let result = await this.$store.dispatch('order/addOrder', order);
    ...
    getdyid(){
       ...
       this.tabIndex = index;
        ...
        axios.get('/static/cinema_data.json', { baseURL: 'http://localhost:8080' })
            .then(response => {
