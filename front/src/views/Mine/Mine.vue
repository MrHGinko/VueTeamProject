<template>
	<div id="mine">
			...
			<div class="m-box" v-for="(item, index) in mineBoxList" :key="index">
				<router-link v-for="(line, index) in item" :key="index" :to="line.to">
					<div class="m-inc border-bottom">
						<div>
							<span class="m-l-icon"> <van-icon name="orders-o" /> </span>
							{{ line.name }}
							<span class="m-r-icon"> {{ returnInfo(line.rTxt) }} <van-icon name="arrow" /> </span>
						</div>
					</div>
				</router-link>
			</div>
		...
		<div>
			<van-popup closeable v-model="isShow" position="right" :style="{width: '50%', height: '100%'}">
				<infoMask v-model="isShow" />
			</van-popup>
		</div>
