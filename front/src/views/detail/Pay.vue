<template>
  <div class="pay">
    <header class="header" ref="header">提交订单
      <i class="iconfont iconarrow-right" @click="backAction"></i>
    </header>
    <content id="content">
      <div class="content">
        <p class="shopName">{{this.ShopInfo.shopName}}
          <i class="iconfont iconarrow-left"></i>
        </p>
        <div class="shopInfo" v-for="(item,index) in commodities" :key="index">
    ...
    <footer class="footer">
      ...
      <span class="discountsPrice">| 已优惠￥{{discountsPrice}}</span>
  ...
  computed: {
    totalPrice(){
      ...
      this.commodities.forEach(item=>{
        totalPrice += item.saleVolume * item.currentPrice
      })
      return totalPrice;
    },
    discountsPrice(){
      ...
      this.commodities.forEach(item=>{
        discountsPrice += item.saleVolume * (item.originPrice - item.currentPrice)
      })
